...
<div id="heading-breadcrumbs">
  <div class="container">
    <div class="row">
      <div class="col-md-7">
          <h1 class="points-banner"> <?= $Lang->get('VOTE__TITLE') ?></h1>
      </div>
      <div class="col-md-5">
        <ul style="margin-top: 18px;text-align: right;">
          <a href="#" data-toggle="modal" data-target="#rewards" class="btn-panier btn"><i class="fa fa-trophy"></i> Récompenses</a>
          <a href="#" data-toggle="modal" data-target="#ranking" class="btn-credits btn"><i class="fa fa-list"></i> Classement</a>
        </ul>
      </div>
    </div>
  </div>
  		  ...
  		  <div class="col-md-12">
    			...
    			<!-- Choose website -->
    				<div style="border:none;text-align: center;" class="panel panel-default panel-step step0">
    				  <div class="panel-heading">
    				    <h3 style="font-family: 'Open Sans', sans-serif;" class="panel-title"><?= $Lang->get('VOTE__STEP_TITLE') ?> 0 : <?= $Lang->get('VOTE__STEP_0_TITLE') ?> <i style="display:none" class="glyphicon glyphicon-ok"></i></h3>
    				  </div>
    				  <div class="panel-body">
    				  	<?php foreach ($websites as $key => $value) { ?>
    				  		<div class="col-md-4">
    							<button style="margin-bottom:5px;" class="btn-send-profil btn btn-primary choose_website btn-block" id="<?= $key ?>"><?= $value['website_name'] ?></button>
    						</div>
    					<?php } ?>
    				  </div>
    				</div>
    			...
    			<!-- Choose username -->
    				<div style="border:none;text-align: center;" class="panel panel-default panel-step step1" style="opacity : 0.5;">
    				  <div class="panel-heading">
    				    <h3 style="font-family: 'Open Sans', sans-serif;" class="panel-title"><?= $Lang->get('VOTE__STEP_TITLE') ?> 1 : <?= $Lang->get('USER__LOGIN') ?> <i style="display:none" id="icon_step1" class="glyphicon glyphicon-ok"></i></h3>
    				  </div>
    				  <div class="panel-body">
    				    <form class="form-inline" data-ajax="true" action="<?= $this->Html->url(array('action' => 'setPseudo')) ?>" data-callback-function="step1">
    					  	...
    					  	<button type="submit" class="btn-send-profil btn btn-primary btn-lg btn-step1 disabled" disabled><?= $Lang->get('USER__LOGIN') ?></button>
    						</form>
    				  </div>
    				</div>
    			...
    			<!-- Vote -->
    				<div style="border:none;text-align: center;" class="panel panel-default panel-step step2" style="opacity : 0.5;">
    				  <div class="panel-heading">
    				    <h3 style="font-family: 'Open Sans', sans-serif;" class="panel-title"><?= $Lang->get('VOTE__STEP_TITLE') ?> 2 : <?= $Lang->get('VOTE__VOTE_ACTION') ?> <i style="display:none" id="icon_step2" class="glyphicon glyphicon-ok"></i></h3>
    				  </div>
    				  <div class="panel-body">
    				  	<p><?= $Lang->get('VOTE__STEP_2_DESC') ?></p>
    				  	<a class="btn-send-profil btn btn-primary btn-block btn-step2 disabled" href="#" target="_blank">Voter</a>
    				  </div>
    				</div>
    			...
    			<!-- Verification of OUT -->
    				<div class="panel panel-default panel-step step3" style="display:none;opacity : 0.5;">
    				  ...
    				  <div class="panel-body">
    					  ...
    					  <button type="submit" class="btn-send-profil btn btn-primary btn-step3 disabled"><?= $Lang->get('GLOBAL__SUBMIT') ?></button>
    					</form>
    				  </div>
    				</div>
    			...
    			<div style="border:none;text-align: center;" class="panel panel-default panel-step step4" style="opacity : 0.5;">
    			  <div class="panel-heading">
    			    <h3 style="font-family: 'Open Sans', sans-serif;" class="panel-title"><?= $Lang->get('VOTE__STEP_TITLE') ?> <span id="step4_title">3</span> : <?= $Lang->get('VOTE__REWARDS_TITLE') ?> <span class="sec"></span><i style="display:none" id="icon_step4" class="glyphicon glyphicon-ok"></i></h3>
    			  </div>
    			  <div class="panel-body">
    			  	<p><?= $Lang->get('VOTE__STEP_4_DESC') ?></p>
    			  	<button class="btn-send-profil btn btn-primary btn-block btn-step4 disabled" id="now"><?= $Lang->get('VOTE__STEP_4_REWARD_NOW') ?></button>
    			  	<button class="btn-send-profil btn btn-primary btn-block btn-step4 disabled" id="later"><?= $Lang->get('VOTE__STEP_4_REWARD_LATER') ?></button>
    			  </div>
